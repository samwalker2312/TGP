...
class minimiser(object):
    ...
    def chisq(self, a, b, c, d):
        self.y = a*np.sin(b*self.time + c) + d
        chisq = np.sum(((self.y - self.vel)/self.err)**2.)
        return chisq

    def minchisq(self):
        self.m = minuit(self.chisq, a = 100, b = .001, c = 10, error_a = .01,\
                        error_b = .00001, error_c = 1, d = 0., error_d=.0001 errordef = 1)
        fmin, param = self.m.migrad()
        self.mina = self.m.values[0]
        self.minb = self.m.values[1]
        self.minc = self.m.values[2]
        self.mind = self.m.values[3]
    ...
    def plot(self):
        ...
        ycalc = self.mina*np.sin(self.minb*x + self.minc) + self.mind
        plt.plot(x, ycalc, color='black')
        ax2 = plt.subplot(212)
        ycalc = self.mina*np.sin(self.minb*self.time + self.minc) + self.mind
